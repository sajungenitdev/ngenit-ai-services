...
export default function CookiePolicyPage() {
    ...
    const [policy, setPolicy] = useState<CookiePolicy>({
        title: "Cookie Policy",
        content: "",
        status: "draft",
        version: 1,
    ...
    const [versions, setVersions] = useState<CookiePolicy[]>([]);
    ...
    const fetchPolicy = async () => {
        try {
            setLoading(true);
            const data = await apiFetch("/cookie-policy");
    ...
    const fetchVersions = async () => {
        try {
            const data = await apiFetch("/cookie-policy/versions");
            if (data.success) {
    ...
    const handlePublish = async () => {
        ...
        setPublishing(true);
        try {
            const data = await apiFetch(`/cookie-policy/${policy._id}/publish`, {
                method: "PATCH",
            });
            ...
            if (data.success) {
                setPolicy(data.data);
                await fetchVersions();
                toast.success("🎉 Cookie policy published successfully!");
    ...
    const handleArchive = async () => {
        ...
        setArchiving(true);
        try {
            const data = await apiFetch(`/cookie-policy/${policy._id}/archive`, {
                method: "PATCH",
            });
    ...
    const handleDelete = async () => {
        ...
        try {
            const data = await apiFetch(`/cookie-policy/${policy._id}`, {
                method: "DELETE",
            });
    ...
    const loadVersion = async (version: number) => {
        try {
            const data = await apiFetch(`/cookie-policy/version/${version}`);
            if (data.success) {
                setPolicy(data.data);
                setUnsavedChanges(false);
                    ...
                    <div className="bg-white rounded-2xl border border-gray-200 shadow-2xl w-full max-w-md p-6">
                        <div className="text-center">
                            ...
                            <p className="text-gray-500 text-sm mb-6">
                                This will make the cookie policy publicly visible on your website.
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
                    <div className="bg-white rounded-2xl border border-gray-200 shadow-2xl w-full max-w-md p-6">
                        <div className="text-center">
                            ...
                            <p className="text-gray-500 text-sm mb-6">
                                Are you sure you want to delete this cookie policy? This action
                                cannot be undone.
                            </p>
                            <div className="flex gap-3">
                                <button
