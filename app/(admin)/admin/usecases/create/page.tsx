    ...
    useEffect(() => {
        const fetchDropdownData = async () => {
            try {
                ...
                const industryNames = industriesData.map(ind => ind.name);
                const serviceNames = servicesData.map(ser => ser.name);

                setIndustries(industryNames);
                setServices(serviceNames);
            } catch (error) {
                console.error("Error fetching dropdown data:", error);
                setError("Failed to load industries or services. Please refresh the page.");
            } finally {
                setLoading(false);
            }
        };
        ...
        return (
            <div className="flex items-center justify-center h-64">
                <div className="text-center">
                    ...
                    <p className="text-grey-400 mt-4">Loading options...</p>
                </div>
            </div>
        );
    }
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-grey-400 text-sm">
                        Add a new real-world AI use case example
                    </p>
                </div>
