...
export default function AiServicesMenus({ isOpen }: ServiceDropdownProps) {
  return (
    <div
      className={`
    ...
    >
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <Link
          href="/services"
          className="text-cyan font-semibold text-sm hover:text-cyan-light transition-colors"
        >
          View All →
        </Link>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2">
        {SERVICES.map((s) => (
          <Link
            href={`/service/${s.id}`}
            key={s.id}
            className="flex items-start gap-3 p-3.5 rounded-xl transition-all duration-200 hover:bg-off-white cursor-pointer"
          >
            ...
            <div>
              <h4 className="text-navy font-semibold text-sm mb-0.5">
                {s.name}
              </h4>
